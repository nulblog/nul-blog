...
import com.gudao.common.utils.bean.BeanValidators;
import com.gudao.common.exceptions.ServiceException;
import com.gudao.common.utils.general.StringChangeUtils;
import com.gudao.common.utils.spring.SpringUtils;
import com.gudao.system.domain.Post;
import com.gudao.system.domain.UserPost;
import com.gudao.system.domain.UserRole;
import com.gudao.system.mapper.*;
import com.gudao.system.service.IConfigService;
import com.gudao.system.service.IDeptService;
...
public class UserServiceImpl implements IUserService {
    ...
    public boolean checkUserNameUnique(User user) {
        Long userId = StringChangeUtils.isNull(user.getUserId()) ? -1L : user.getUserId();
        User info = userMapper.checkUserNameUnique(user.getUserName());
        if (StringChangeUtils.isNotNull(info) && info.getUserId().longValue() != userId.longValue()) {
    ...
    public void checkUserAllowed(User user) {
        if (StringChangeUtils.isNotNull(user.getUserId()) && user.isAdmin()) {
    ...
    public void checkUserDataScope(Long userId) {
        if (!User.isAdmin(SecurityUtils.getUserId())) {
            ...
            if (StringChangeUtils.isEmpty(users)) {
    ...
    public void insertUserPost(User user) {
        ...
        if (StringChangeUtils.isNotEmpty(posts)) {
            // 新增用户与岗位管理
            List<UserPost> list = new ArrayList<UserPost>(posts.length);
    ...
    public void insertUserRole(Long userId, Long[] roleIds) {
        if (StringChangeUtils.isNotEmpty(roleIds)) {
            // 新增用户与角色管理
    ...
    public String importUser(List<User> userList, Boolean isUpdateSupport, String operName) {
        if (StringChangeUtils.isNull(userList) || userList.size() == 0) {
        ...
        for (User user : userList) {
            try {
                ...
                if (StringChangeUtils.isNull(u)) {
                    BeanValidators.validateWithException(validator, user);
                    deptService.checkDeptDataScope(user.getDeptId());
                    String password = configService.selectConfigByKey("user.initPassword");
